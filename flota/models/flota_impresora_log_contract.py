...
class FlotaImpresoraLogContract(models.Model):
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _name = 'flota.impresora.log.contract'
    _description = 'Impresora Contract'
    ...
    impresora_id = fields.Many2one('flota.impresora', 'Impresora', required=True, check_company=True)
    cost_subtype_id = fields.Many2one('flota.service.type', 'Type', help='Cost type purchased with this cost', domain=[('category', '=', 'contract')])
    ...
    user_id = fields.Many2one(
        ...
        string='Responsible',
        default=lambda self: self.env['flota.impresora'].browse(self._context.get('active_id')).manager_id,
        index=True)
    ...
    purchaser_id = fields.Many2one(related='impresora_id.driver_id', string='Driver')
    ...
    service_ids = fields.Many2many('flota.service.type', string="Included Services")

    @api.depends('impresora_id.name', 'cost_subtype_id')
    ...
    def _compute_has_open_contract(self):
        ...
        open_contracts = self.env['flota.impresora.log.contract'].search([
            ('impresora_id', 'in', self.impresora_id.ids),
            ('state', '=', 'open'),
            ('expiration_date', '>=', today)
        ])
        for log_contract in self:
            log_contract.has_open_contract = log_contract.impresora_id in open_contracts.impresora_id
    ...
    def write(self, vals):
        res = super(FlotaImpresoraLogContract, self).write(vals)
        ...
        if vals.get('expiration_date') or vals.get('user_id'):
            self.activity_reschedule(['flota.mail_act_flota_contract_to_renew'], date_deadline=vals.get('expiration_date'), new_user_id=vals.get('user_id'))
        return res
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        # It manages the state of a contract, possibly by posting a message on the impresora concerned and updating its status
        ...
        reminder_activity_type = self.env.ref('flota.mail_act_flota_contract_to_renew', raise_if_not_found=False) or self.env['mail.activity.type']
        ...
        for contract in nearly_expired_contracts:
            contract.activity_schedule(
                'flota.mail_act_flota_contract_to_renew', contract.expiration_date,
