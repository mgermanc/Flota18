...
class FlotaImpresoraModel(models.Model):
    _name = 'flota.impresora.model'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _description = 'Model of a impresora'
    _order = 'name asc'

    name = fields.Char('Model name', required=True, tracking=True)
    brand_id = fields.Many2one('flota.impresora.model.brand', 'Manufacturer', required=True, tracking=True)
    category_id = fields.Many2one('flota.impresora.model.category', 'Category', tracking=True)
    vendors = fields.Many2many('res.partner', 'flota_impresora_model_vendors', 'model_id', 'partner_id', string='Vendors')
    image_128 = fields.Image(related='brand_id.image_128', readonly=True)
    active = fields.Boolean(default=True)
    impresora_type = fields.Selection([('car', 'Car'), ('bike', 'Bike')], default='car', required=True, tracking=True)
    transmission = fields.Selection([('manual', 'Manual'), ('automatic', 'Automatic')], 'Transmission', tracking=True)
    impresora_count = fields.Integer(compute='_compute_impresora_count', search='_search_impresora_count')
    ...
    impresora_properties_definition = fields.PropertiesDefinition('Impresora Properties')
    ...
    def _compute_impresora_count(self):
        group = self.env['flota.impresora']._read_group(
            [('model_id', 'in', self.ids)], ['model_id'], aggregates=['__count'],
        )
        count_by_model = {model.id: count for model, count in group}
        for model in self:
            model.impresora_count = count_by_model.get(model.id, 0)

    @api.model
    def _search_impresora_count(self, operator, value):
        if operator not in ['=', '!=', '<', '>'] or not isinstance(value, int):
            raise NotImplementedError(_('Operation not supported.'))
        flota_models = self.env['flota.impresora.model'].search([])
        if operator == '=':
            flota_models = flota_models.filtered(lambda m: m.impresora_count == value)
        elif operator == '!=':
            flota_models = flota_models.filtered(lambda m: m.impresora_count != value)
        elif operator == '<':
            flota_models = flota_models.filtered(lambda m: m.impresora_count < value)
            ...
            flota_models = flota_models.filtered(lambda m: m.impresora_count > value)
        return [('id', 'in', flota_models.ids)]

    def action_model_impresora(self):
        self.ensure_one()
        context = {'default_model_id': self.id}
        if self.impresora_count:
